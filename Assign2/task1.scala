...
object task1 {
  ...
  def mapPhase2(partition: List[List[String]],candidatePairs: List[List[String]]) : Iterator[(List[String],Long)] = {
    ...
    val main = row.map(x => (x(i), x(j))).reduceByKey((x, y) => (x + "," + y))
